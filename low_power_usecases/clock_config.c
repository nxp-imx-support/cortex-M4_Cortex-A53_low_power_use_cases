...
//    CLOCK_SetRootMux(kCLOCK_RootAudioAhb, kCLOCK_AudioAhbRootmuxSysPll1); /* switch AUDIO AHB to SYSTEM PLL1 */
    ...
    CLOCK_SetRootMux(kCLOCK_RootUart4, kCLOCK_UartRootmuxOsc24M); /* Set UART source to Osc24 MHz */
    CLOCK_SetRootDivider(kCLOCK_RootUart4, 1U, 1U);

#if (USE_CASE_WITH_TIMER == 1)
     CLOCK_SetRootMux(TIMER_CLOCK, kCLOCK_GptRootmuxOsc24M);      /* Set GPT source to Osc24 MHZ */
    CLOCK_SetRootDivider(TIMER_CLOCK, 1U, 1U);
#endif //USE_CASE_WITH_TIMER
   
#if (USE_CASE_WITH_I2C == 1)
    CLOCK_SetRootMux(I2C_CLOCK, kCLOCK_I2cRootmuxOsc24M);      /* Set I2C source to Osc24 MHZ */
    CLOCK_SetRootDivider(I2C_CLOCK, 1U, 1U);
